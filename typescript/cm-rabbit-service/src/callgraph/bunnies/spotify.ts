...
The placeholder appears with brackets around it, like [the from the search function]. If there are no brackets, the query is complete and is not a placeholder. In that case, you should not change the query.
...
Note: if the query contains no artist name, and previousBunnyContext contains no artist name, you should not add an artist name to the query.
...
Updated song query (Song name by artist):`;
      ...
      if (newQuery.trim() !== "") {
        ...
        */
        // TODO (Peiyuan): log tokens back to DB.
        if (await kernel.spotifySession.isLoginStorageAvailableServer(scope)) {
          await kernel.spotifySession.handleClientMessage(scope, {
            playerControl: {
              playSearchTop: {
                query: newQuery,
              },
          ...
          // TODO (Peiyuan): handle this case
          console.log("Spotify login storage not available.")
        }
      }
