...
export async function runCoreBunny(
    ...
    let userDoc: UserStorage = await getUserDocCreateIfNotExist(scope, modelClient, language, userId);
...
Updated query:`;
    ...
    userDoc = await getUserDocCreateIfNotExist(scope, modelClient, language, userId);
    let currentIntention = userDoc?.current_intention.toLowerCase() ?? "";
    if (currentIntention.includes("empty") || currentIntention.length == 0) {
        console.log("Intention is empty.") // TODO: not adding it to mailbox, but currently intention is not good enough.
    } 
    var userDoc_ = {
        ...userDoc,
        bunny_mailbox: [{ bunnyId: bunnyId, result: result, flush: false, intentionId: config.intentionId }],
    }
