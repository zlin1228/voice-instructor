    ...
    try {
      ...
      await Scope.with(undefined, [], async (scope: any) => {
        ...
        kernel.sharedParameter = {
          ...
          intentionId: kernel.currentIntentionId,
        }
        ...
        launchBackgroundScope(scope, async (scope: Scope) => {
          try {
            while (kernel.openAiInstructClient == null) {
              await sleepSeconds(scope, 0.2)
              console.log("Waiting for openAiInstructClient to be initialized...")
            }
            ...
            console.log("Error updating intention by assistant response. This should be normal for an abort.")
          }
        })
        ...
        if (intentionCallgraphSchedule.steps.length > 0) {
          if (isScheduleSingleton(intentionCallgraphSchedule)) {
            ...
            console.log(
              "Callgraph: Singleton. Handle step by step in change stream.", singleton
            )
