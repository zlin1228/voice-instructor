...
import { streamingConversation } from "../conversation/conversation.js"
import { template } from "../conversation/conversation-prompt.js"
...
import {
  ...
  openAiModel_Gpt35_Finetuned_CoreNonexec,
...
export class Kernel {
  ...
  finetunedOpenAillmClient: OpenAiLlmClient | null = null
  ...
  anthropicLlm2Client: AnthropicLlmClient | null = null
  ...
  private constructor(
    ...
    let { cancel, attachment } = buildAttachmentForCancellation(true)
      ...
      await runCancellableScope(scope, async (scope: Scope, cancel: any) => {
        ...
        userDocOperationBroadcast.listen(scope, (userOperationDoc: UserStorage) => {
          if (userOperationDoc._id === this.userId) {
            let newBunnyMailbox = (userOperationDoc.bunny_mailbox as unknown as { bunnyId: string, result: string, flush: boolean, intentionId: string }[]) ?? []
            let newScheduleStep = userOperationDoc.current_callgraph.current_step
    ...
    launchBackgroundScope(scope, async (_: any) => {
      this.finetunedOpenAillmClient = await buildOpenAiLlmClient(scope, {
        apiKey: "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",
        model: openAiModel_Gpt35_Finetuned_CoreNonexec,
        completionLogger: await buildLlmCompletionLogger(
          await LlmModelClient.build(
            scope,
            "mongodb+srv://info:eLVGtLSn2qmKZAgp@rabbit-os.fu6px.mongodb.net/?retryWrites=true&w=majority",
      ...
      const messages = template("", "Assistant: Hi!", "What's up?", "User", "Assistant", "", "", "", "America/Los_Angeles");
      console.log(encode_cl100k_base(messages[0]?.content ?? "").length)
      let buf = ""
      for await (const result of this.finetunedOpenAillmClient.openAichatCompletionStream(scope, {
          messages: messages,
          stopSequences: [],
          temperature0to1: 0.0,
          maxTokens: 96,
    ...
    launchBackgroundScope(scope, async (_: any) => {
      this.anthropicLlm2Client = await buildAnthropicLlmClient(scope, {
        apiKey:
          "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",
...
`
  ...
  async handleTextMessage(requestText: string, scope: Scope): Promise<void> {
    ...
    if (requestText.trim().length > 0) {
      ...
      let assistantName: string = userDoc?.assistant_name ?? "Assistant"
      ...
      let firstFragmentDelivered = { current: false }

      try {
        let attachment = this.cancelAttachment
        await Scope.with(undefined, [attachment], async (scope: any) => {
          while (this.openAillmClient == null || this.anthropicLlm2Client == null || this.finetunedOpenAillmClient == null) {
            await sleepSeconds(scope, 0.1)
            console.log("Waiting for openAillmClient or anthropicLlm2Client or finetunedOpenAillmClient to be initialized...")
          }
          const llmClient = this.openAillmClient
          const finetunedLlmClient = this.finetunedOpenAillmClient
          const anthropicLlm2Client = this.anthropicLlm2Client

          let chatPieceTime = 0
          let assistantResponse = ""
          const language = this.language

          let callgraphDetermined = false
          let chunks = 0
          let embeddingClient = this.embeddingClient
          let vectorDBClient = this.milvusClient
          let collectionName = this.milvusCollectionName
          ...
          let memory = semanticMemory.concat(recallMemory)
          ...
          for await (let chatAccumulator of chatFragmentIterToChatPieceIter(
            (async function* () {
            ...
            let chatPiece = chatAccumulator.fragment
            let fullyAccumulatedChatPiece = chatAccumulator.fragmentLookAhead
            let expertAccumulator = chatAccumulator.expertAccumulator
            ...
            if (
              ...
              "How can I assist you further today?" === chatPiece.trim() ||
            ...
            if (chunks > 0 && userDoc != undefined && !callgraphDetermined) {
              callgraphDetermined = true
              console.log("Launching callgraph pattern after the first fragment...")
              console.log("[Current response]: ", assistantResponse)
              console.log("[fully accumulated]: ", fullyAccumulatedChatPiece)
              console.log("[expertAccumulator]: ", expertAccumulator)
              launchBackgroundScope(scope, async (_: Scope) => {
