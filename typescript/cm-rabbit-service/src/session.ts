...
export async function handleSession(
  ...
  let spotifyStorageFetcher: (scope: Scope) => Promise<string> = async (scope: Scope) => {
    throw new Error("Spotify storage fetcher not initialized yet.")
  }
  let spotifyStoragePusher: (scope: Scope, storage: string) => Promise<void> = async (scope: Scope) => {
    throw new Error("Spotify storage pusher not initialized yet.")
  }
  const spotifySession = new SpotifySession(
    ...
    async (scope) => await spotifyStorageFetcher(scope),
    async (scope, storage) => await spotifyStoragePusher(scope, storage),
  )
  ...
  const handleGlobalInitialize = async (
    ...
    spotifyStorageFetcher = async (scope) => {
      const userDoc = await modelClient.userStorageCollections
        .get(session.language)
        ?.getById(scope, validUserId)
      if (userDoc === undefined) {
        throw new Error("User doc not avaialble")
      }
      const spotifyStorage = userDoc.spotifyStorage
      // TODO (Peiyuan): check storage validity
      if (spotifyStorage === undefined) {
        throw new Error("User Spotify storage not avaialble")
      }
      return spotifyStorage
    }
    spotifyStoragePusher = async (scope, storage) => {
      const userDoc = await modelClient.userStorageCollections
        .get(session.language)
        ?.getById(scope, validUserId)
      if (userDoc === undefined) {
      ...
      await modelClient.userStorageCollections
        .get(session.language)?.createOrReplace(scope, {
          ...userDoc,
          spotifyStorage: storage
        })
    }
    ...
    serverMessageQueue.pushBack({
      ...
      value: {
        kernel: {
          initializeSpotify: (await spotifySession.isLoginStorageAvailableServer(scope)) ?
            (await spotifySession.getLoginStorage(scope)) : "",
        }
      },
    })
  }
            ...
            if (jsonMessage.global !== undefined) {
              ...
              if (jsonMessage.kernel.userText !== undefined) {
                if (kernelInstance !== undefined) {
                  kernelInstance.currentIntentionId = randomUUID().toString()
                  launchBackgroundScope(scope, async (scope: Scope) => {
