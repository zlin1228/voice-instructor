  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Please visit{" "}
          <span style={{ textDecoration: "underline" }}>demo.rabbit.tech</span>{" "}
          on desktop browser.
        </div>
      </div>
      <div
...
function Os2App(props: { mode: string }) {
  ...
  const spotifyStorageDebug = useDebugValueHas("spotify-storage")
  ...
  const handleKeyDown = (event: any) => {
    if (event.code === "Space" && textmodeActiveRef.current === false) {
      if (spacePressedRef.current === false) {
        ...
        previousVolumeRef.current = previousVolume
        console.log("Storing volume: " + previousVolume)
        setVolume(0.1)
  ...
  const handleKeyUp = (event: any) => {
    if (event.code === "Space" && textmodeActiveRef.current === false) {
      ...
      console.log("Restoring volume: " + previousVolumeRef.current)
  ...
  }, [sessionControllerRef, volumeRef, previousVolumeRef, volume])
  ...
  useEffect(
    () =>
      runEffectScope(async (scope) => {
        ...
        sessionControllerRef.current.phase.value.connect(
          token,
  ...
  const initializeSpotify = useCallback(
    async (scope: Scope, loginStorage: string) => {
      if (
        pushSpotifyClientMessageRef.current === undefined ||
        spotifyServerMessageBroadcastRef.current === undefined
      ) {
      ...
      const spotifyLoginStorage = await (async () => {
        if (spotifyStorageDebug) {
          return undefined
        }
        if (loginStorage.length === 0) {
          appendDebugLog(
            spotifyLogStyle,
            "You haven't logged into Spotify yet. Allocating your Spotify Login."
          )
          ...
          writeSpotifyLoginStorage(spotifyLoginStorageKey, loginStorage)
          return spotifyLoginStorageRef.current
        }
      })()
      const controllerConnected = await (async () => {
    ...
    () =>
      runEffectScope(async (scope) => {
        ...
        const handleControlMessage = (serverMessage: Os2ServerMessage) => {
          ...
          const initializeSpotifyMsg = serverMessage.kernel?.initializeSpotify
          if (initializeSpotifyMsg !== undefined) {
            appendDebugLog(
              spotifyLogStyle,
              `You asked to initialize Spotify.`
            )
            flyingPromise(async () => {
              await Scope.with(undefined, [], async (scope) => {
                await initializeSpotify(scope, initializeSpotifyMsg)
              })
            })
          }
        }
  ...
  */
  ...
  const onTouchStart = (e: React.TouchEvent<HTMLDivElement>) => {
    if (
      ...
      if (textmodeActiveRef.current === false) {
        if (spacePressedRef.current === false) {
          if (sessionControllerRef.current.phase.kind === "connected") {
            sessionControllerRef.current.phase.value.pushClientMessage({
              kind: "json",
  ...
  // enable mobile on dev.rabbit.tech
    ...
    >
      ...
      >
        ...
        {connectedState !== undefined &&
          pushSpotifyClientMessage !== undefined &&
          spotifyServerMessageBroadcast !== undefined &&
          (spotifyLoginStorage !== undefined || spotifyStorageDebug) && (
            <SpotifyPlayerPanel
              controller={spotifyPlayerController}
        ...
        >
          ...
          {textmodeActive ? (
            <img
              width={40}
              style={{ cursor: "pointer", marginLeft: 5, marginBottom: 3 }}
              src="https://storage.googleapis.com/rabbit-public/svg/rabbit_os_assets_switches__text_on.svg"
              onClick={handleTextmodeActive}
            ></img>
          ) : (
            <img
              role="button"
              ...
              width={40}
              style={{ cursor: "pointer", marginLeft: 5, marginBottom: 3 }}
              src="https://storage.googleapis.com/rabbit-public/svg/rabbit_os_assets_switches__text_off.svg"
              onClick={handleTextmodeActive}
            ></img>
          )}
        </div>
        ...
        >
          {listening
            ? textmodeActive
              ? "Turn off text mode to talk."
              : spacePressed
              ? "You are talking ..."
              : "Press and hold the space bar to talk."
        ...
        >
          <a href="https://twitter.com/rabbit_hmi" target="_blank">
            <img
              height={17}
              style={{
                marginTop: 21,
              ...
              src={BASEURL + "/twitter_logo_white.png"}
            ></img>
          </a>
          <SignOutButton />
        </div>
