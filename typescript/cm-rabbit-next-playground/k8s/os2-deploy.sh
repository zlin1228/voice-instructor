...
main() {
  ...
  local yaml_path="$cluster_path"/yaml/"$namespace"/os2-base/"$deployment_name".deploy.yaml
  ...
  "$cluster_path"/kubectl.sh rollout status -n "$namespace" -w deployment/"$deployment_name"
}
